...
- ✅ **Протокол Cold Start** - экономия 60% токенов при перезагрузках сессий
...
- **Без протокола:** Читает ВСЕ файлы → ~15-20k токенов (~$0.15-0.20)
- **С протоколом:** Читает ТОЛЬКО нужные файлы → ~6-8k токенов (~$0.05-0.08)
- **Результат:** **~60% экономия токенов** при каждой перезагрузке! 🚀

### Как это работает

**Этап 1: Quick Status Check (~500 токенов)**
1. Читает PROJECT_INTAKE.md (только первые 20 строк)
2. Проверяет: `Status`, `Migration Status`, Project Name
3. Решает что читать дальше на основе статуса

**Этап 2: Context Loading (~5-7k токенов)**
- IF проект заполнен → Читает полный PROJECT_INTAKE.md + BACKLOG.md
- IF нужен код → Читает ARCHITECTURE.md + SECURITY.md
- IF миграция завершена → **Пропускает MIGRATION_REPORT.md** автоматически

**Этап 3: Never Unless Asked**
- ❌ MIGRATION_REPORT.md (только по запросу пользователя)
