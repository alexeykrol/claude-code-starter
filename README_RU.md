...
### 🎯 Умный скрипт установки (v1.2.0)

**НОВОЕ:** Супер-простая установка для любого проекта за 3 шага!

**Было (5 шагов, легко ошибиться):**
```bash
...
cp -r ../claude-code-starter/Init/.claude .  # Легко забыть!
```

**Стало (1 команда):**
```bash
bash init-project.sh  # Готово!
```

**Что делает скрипт:**
- ✅ Спрашивает подтверждение что вы в нужной папке
- ✅ Распаковывает шаблоны
- ✅ Определяет новый или существующий проект
- ✅ **Новый проект:** Автоматически устанавливает шаблоны
- ✅ **Legacy проект:** Готовит к миграции

**Зачем это нужно:**
- Устраняет ошибки ручного копирования (легко забыть .claude/ или .env.example)
- Упрощает установку для начинающих
- Автоматически выбирает правильный workflow
