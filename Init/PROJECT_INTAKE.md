...
2. **Простота тестирования:** Каждый модуль = отдельная задача = легко проверить
3. **Параллельная разработка:** Можно делать разные модули одновременно
4. **Дешевизна:** Меньше контекста = меньше стоимость запросов к ИИ

**Принцип:** Приложение = набор маленьких кубиков (модулей), каждый решает узкую задачу

**Типичные модули:**
- Аутентификация (отдельный модуль)
- База данных (отдельный модуль)
- Каждый экран интерфейса (отдельный модуль)
- Каждая бизнес-функция (отдельный модуль)
- API / Backend (отдельный модуль)
