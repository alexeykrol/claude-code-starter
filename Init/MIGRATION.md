...
**Решение (до финализации):**
```bash
# Просто удалите Init/ и archive/
rm -rf Init/ archive/ MIGRATION_REPORT.md CONFLICTS.md

# Восстановите из git (если был коммит перед миграцией)
git checkout HEAD~1 -- .
```

**Решение (после финализации):**
```bash
# Откатите последний коммит
git reset --soft HEAD~1

# Восстановите legacy файлы из archive/
