...
## 🏁 Sprint Completion Protocol (ОБЯЗАТЕЛЬНО!)

> **🚨 КРИТИЧНО:** Читай этот раздел ПЕРЕД тем как сказать "готово"!

### КОГДА: Последняя задача завершена

**ДО того как сказать пользователю "готово", ОБЯЗАТЕЛЬНО:**

1. ✅ **Прочитай CLAUDE.md** → Чеклист завершения фазы (строки 78-110)
2. ✅ **Обнови BACKLOG.md:**
   - Отметить выполненные задачи [x]
   - Обновить статус фазы (In Progress → ✅ Complete)
   - Пересчитать таблицу прогресса (%)
3. ✅ **Обнови SNAPSHOT.md:**
   - Обновить дату последнего обновления
   - Изменить статус фазы на ✅
   - Добавить завершенные задачи в "✅ Завершенные задачи"
   - Обновить "🔜 Следующий этап"
4. ✅ **Обнови CLAUDE.md:**
   - Раздел "🔄 Текущее состояние проекта"
   - Отметить завершенные задачи
   - Указать следующий шаг
5. ✅ **Git commit** с обновлением мета-файлов
...
📋 Перед завершением спринта обновлю мета-файлы:
   - BACKLOG.md (задачи, прогресс)
   - SNAPSHOT.md (статус, дата)
   - CLAUDE.md (текущее состояние)

Обновить сейчас? (Рекомендуется ДА)
```

### 🎯 Быстрый доступ:

**Используй `/finalize`** (если команда доступна) для автоматической проверки

**ТОЛЬКО после обновления мета-файлов → можно сказать "готово"!**
...
- ❌ Не упомянули SNAPSHOT.md и модульный фокус в features
